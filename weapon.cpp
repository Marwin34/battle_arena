...
void Weapon::define(std::string oName, int xx, int yy)
{
    if (oName == "glock") {
        ...
        texture.loadFromImage(image,
            sf::IntRect(
                frame * image.getSize().x / 2, 0, image.getSize().x / 2, image.getSize().y));
...
void Weapon::update(int xx, int yy, float angle)
{
    ...
    texture.loadFromImage(image,
        sf::IntRect(frame * image.getSize().x / 2, 0, image.getSize().x / 2, image.getSize().y));
    sprite.setRotation(angle);
    sprite.setPosition(x, y);
}

void Weapon::draw(sf::RenderWindow* win) { win->draw(sprite); }

int Weapon::getFrame() { return frame; }

int Weapon::getAttackSpeed() { return attackSpeed; }

int Weapon::getX() { return gX; }

int Weapon::getY() { return difY; }