...
bool Airdrop::colliding(int boxX, int boxY, Player& player) {
	...
	if (r < 20) {
		if (kind == Type::MEDKIT) {
			if (player.hp + amount > player.getMaxHp())player.hp = player.getMaxHp();
...
void Airdrop::update(sf::Time timer) {
	//"NAPRAWIAMY" POZYCJE NA OSI Y
	if (repair) {
		y = (y / height) * height;
		repair = false;
	}

	//G£OWNY WARUNEK AKTUALIZACJI PACZKI
...
void Airdrop::draw(sf::RenderWindow* win) {
	win->draw(sprite);
}
