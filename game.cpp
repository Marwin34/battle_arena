...
Game::Game()
{
    ...
    window.create(
        sf::VideoMode(SCRN_WIDTH, SCRN_HEIGTH), "Battle Arena", sf::Style::Default, settings);
    ...
    while (window.isOpen()) {
        ...
        if (window.hasFocus()) {
            ...
            if (sf::Keyboard::isKeyPressed(sf::Keyboard::Escape))
                canQuit = true;
            if (gameStatus == endingScrren)
                gui.saveHighScore(player1.getScore());
            ...
            if ((gameStatus == playing && lastGameStatus == restartScreen)
                || (gameStatus == playing && lastGameStatus == endingScrren))
                reInitialize();
            ...
            if (gameStatus == playing) {
                ...
                if (frames % 800 == 0) {
                    ...
                    for (unsigned int i = 0; i < level[mapIndex].bricks.size(); i++) {
                        if (level[mapIndex].bricks[i].y == 0) { // CHECK IF AIRDROP SPAWN IN WALL
                            if (x <= level[mapIndex].bricks[i].x * BLOCK_SIZE
                                        + level[mapIndex].bricks[i].width
                                && level[mapIndex].bricks[i].x * BLOCK_SIZE <= x + BLOCK_SIZE
                                && 0 + BLOCK_SIZE <= level[mapIndex].bricks[i].y
                                        + level[mapIndex].bricks[i].height
                                && level[mapIndex].bricks[i].height <= 0 + BLOCK_SIZE)
                                a++;
                        }
                    }
                    if (a == 0)
                        airdropPacks.push_back(
                            Airdrop(x, 0, (std::rand() % difficult) + 0, ammoText, kitText));
                }

                if (sf::Mouse::isButtonPressed(sf::Mouse::Left) && player1.canFire
                    && player1.ammunition[player1.activeGun] > 0) {
                    Bullet bullet1;
                    bullet1.create(pXnormal + player1.weapon.getX(), pY,
                        aiming(pX + player1.weapon.getX(), pY, mousePos.x, mousePos.y),
                        player1.getName(), bulletText, player1.weapon.dmg, player1.activeGun);
                ...
                for (unsigned int i = 0; i < bulletList.size(); i++) {
                    if (bulletList[i].ttl > 10 + player1.activeGun * 2) {
                        bulletList[i].toRemove = true;
                    }
                    for (unsigned int j = 0; j < level[mapIndex].bricks.size(); j++) {
                        if (bulletList[i].colliding(level[mapIndex].bricks[j].x,
                                level[mapIndex].bricks[j].y, level[mapIndex].bricks[j].width,
                                level[mapIndex].bricks[j].height)) {
                            bulletList[i].toRemove = true;
                        }
                    }

                    if (bulletList[i].colliding(player1.x / BLOCK_SIZE, player1.y / BLOCK_SIZE,
                            player1.width, player1.height)
                        && bulletList[i].originName != player1.getName()) {
                        if (bulletList[i].originName != player1.getName())
                            player1.hp -= bulletList[i].dmg;
                        bulletList[i].toRemove = true;
                    }

                    for (unsigned int j = 0; j < index; j++) {
                        if (bulletList[i].colliding(wsk[j]->x / BLOCK_SIZE, wsk[j]->y / BLOCK_SIZE,
                                wsk[j]->width, wsk[j]->height)
                            && bulletList[i].originName == player1.getName()) {
                            bulletList[i].toRemove = true;
                ...
                for (unsigned int i = 0; i < level[mapIndex].bricks.size(); i++) {
                    player1.collisionDetect(level[mapIndex].bricks[i].x,
                        level[mapIndex].bricks[i].y, level[mapIndex].bricks[i].width,
                        level[mapIndex].bricks[i].height);
                }

                for (unsigned int i = 0; i < index; i++) {
                    player1.collisionDetect(wsk[i]->x / BLOCK_SIZE, wsk[i]->y / BLOCK_SIZE,
                        wsk[i]->width, wsk[i]->height);
                }

                // PICKUP COLLISIONS
                for (unsigned int i = 0; i < level[mapIndex].gifts.size(); i++) {
                    float r = sqrt(pow(player1.x - level[mapIndex].gifts[i].x, 2)
                        + pow(player1.y - level[mapIndex].gifts[i].y, 2));
                ...
                // NPC COLLISION LOOP
                for (unsigned int i = 0; i < index; i++) {
                    wsk[i]->collisionDetect(player1.x / BLOCK_SIZE, player1.y / BLOCK_SIZE,
                        player1.width, player1.height);
                    for (unsigned int j = 0; j < level[mapIndex].bricks.size(); j++) {
                        wsk[i]->collisionDetect(level[mapIndex].bricks[j].x,
                            level[mapIndex].bricks[j].y, level[mapIndex].bricks[j].width,
                            level[mapIndex].bricks[j].height);
                        wsk[i]->LineCast(player1.transformPos(),
                            level[mapIndex].brickTransformPos(j), level[mapIndex].bricks[j].width,
                            level[mapIndex].bricks[j].height);
                    }
                }
                ...
                for (unsigned int i = 0; i < airdropPacks.size(); i++) {
                    for (unsigned int j = 0; j < level[mapIndex].bricks.size(); j++) {
                        airdropPacks[i].collisionDetect(level[mapIndex].bricks[j].x,
                            level[mapIndex].bricks[j].y, level[mapIndex].bricks[j].width,
                            level[mapIndex].bricks[j].height);
                    }
                    ...
                    if (airdropPacks[i].y > level[mapIndex].getHeight() * BLOCK_SIZE)
                        airdropPacks.erase(airdropPacks.begin() + i);
                }

                // PLAYER UPDATE
                player1.update(mainTimer, aiming(pX, pY, mousePos.x, mousePos.y),
                    level[mapIndex].getWidth() * BLOCK_SIZE);
                ...
                for (unsigned int i = 0; i < index; i++) {
                    ...
                    wsk[i]->update(mainTimer, player1.x, player1.y, player1.width, player1.height,
                        aiming(nX, nY, pXnormal, pY));
                    if (wsk[i]->canFire) {
                        Bullet bullet1;
                        bullet1.create(nX, nY, aiming(nX, nY, pXnormal, pY), wsk[i]->getName(),
                            bulletText, 10, 0);
                        bulletList.push_back(bullet1);
                        wsk[i]->canFire = false;
                        wsk[i]->sCnt = 0;
                ...
                if (player1.y == level[mapIndex].portal.y) {
                    if (player1.x == level[mapIndex].portal.x) {
                        if (level[mapIndex].completed) {
                            if (difficult < MAP_COUNT)
                                difficult++;
                            if (mapIndex + 1 > MAP_COUNT - 1)
                                gui.setMode("quit");
                            else {
                                mapIndex++;
                                initialize();
                ...
                gui.updateWhilePlaying(player1.activeGun, player1.getMaxHp(), player1.getHp(),
                    &player1.ammunition[0], player1.getScore(), viewDiffs.x, mapIndex);
                window.setView(view);
            }
            lastGameStatus = gameStatus;
            if (player1.hp <= 0 || player1.y > SCRN_HEIGTH)
                gui.setMode("restart");
            gui.activeButtons(mousePos.x + viewDiffs.x - 400, mousePos.y, pushback, gameStatus);
            cursor.setPosition(static_cast<float>(mousePos.x + viewDiffs.x - SCRN_WIDTH / 2),
