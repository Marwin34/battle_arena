...
void Gui::initialize(int sWidth,int sHeight){
    //INICJALIZACJA SKŁADOWYCH ELEMENTÓW GUI
    loadHighScores();
    ...
    instruction.setPositionAndValue(5,540," INSTRUKCJA: \n PORUSZANIE - strzalki lub W,A,D \n STRZELANIE - LPM");
...
void Gui::switchState(int &gameStatus){
    if(mode==sScreen) gameStatus=0;
    if(mode==rScreen) gameStatus=1;
    if(mode==eScreen) gameStatus=2;
    if(mode==cScrren) gameStatus=3;
}

void Gui::setMode(std::string name){
    if(name=="restart")mode=eScreen;
    if(name=="quit")mode=cScrren;
}

void Gui::updateWhilePlaying(int index,int maxHp,int hp,int *aTable,int score,int x,int mapIndex){
    ...
    if(mode==rScreen){
        ...
        int newX=0;
        newX=gunsPosition+x-400; // 400 1/2 D£UGOSCI EKRANU
        for(int i=0;i<6;i++){
        ...
        restartButton.setPosition(x-400,0);//PODAJEMY ILE MA SIE PRZESUN¥NÆ
        ...
        instruction.setPositionAndValue(x+5-400,540," INSTRUKCJA: \n PORUSZANIE - strzalki lub W,A,D \n STRZELANIE - LPM");
...
void Gui::saveHighScore(int eScore){ //eScore = end score
    if(!hSSaved){
        std::ofstream file;
        std::ifstream file2;
        file.open("C:/tmp.txt",std::ios::app);
        file2.open("C:/hs.txt",std::ios::app);
        int fScore; //FILE SCORE
        ...
        //SORTOWANIE BĄBELKOWE BUFORA WYNIKÓW
        ...
        std::cout<<eScore<<std::endl;
        hSSaved=true;
    }

}

void Gui::resetHSflag(){
    hSSaved=false;
}

void Gui::draw(sf::RenderWindow *win){
    //WYSWIETLANIE HUI PODZIELONE DLA ODPOWIEDNICH TRYBÓW
    if(mode==sScreen){
    ...
    if(mode==rScreen){
        ...
        for(int i=0;i<6;i++)ammuntion[i].draw(win);
